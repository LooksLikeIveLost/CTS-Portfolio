...
var _addGhostPos = function (event, ghost) {
  if (!ghost.x) {
    ghost.x = parseInt(event.pageX - _offset(ghost.draggedItem).left, 10);
  }
  if (!ghost.y) {
    ghost.y = parseInt(event.pageY - _offset(ghost.draggedItem).top, 10);
  }
  return ghost;
};
/**
...
var sortable = function (sortableElements, options) {
  ...
  sortableElements.forEach(function (sortableElement) {
    ...
    _on(items, 'dragstart', function (e) {
      ...
      draggingHeight = parseInt(window.getComputedStyle(dragging).height, 10);
    ...
    var debouncedDragOverEnter = _debounce(function (element, pageY) {
      ...
      if (items.indexOf(element) !== -1) {
        var thisHeight = parseInt(window.getComputedStyle(element).height, 10);
